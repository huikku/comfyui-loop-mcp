...
from .subgraph import flatten as flatten_subgraphs
...
def litegraph_to_api(wf: dict, object_info: dict) -> tuple[dict, list[str]]:
    ...
    Subgraphs are EXPANDED first (see subgraph.flatten) — the interior nodes come
    through namespaced `<instance>:<inner>`, wired straight to whatever was on the
    other side of the boundary, so a graph authored with subgraphs runs instead of
    arriving with a hole where the instance was.

    Then: links resolve to [from_node_id, slot], and positional widgets_values map
    to their named inputs in object_info order (handling seed's extra
    control_after_generate value). Returns (api_graph, warnings). Unknown nodes are
    skipped and reported — the loop's node_errors is the safety net; treat the
    result as a draft to validate by executing.
    """
    wf, promoted, warnings = flatten_subgraphs(wf)
    ...
    for n in wf.get("nodes", []):
        ...
        # A widget CONVERTED to an input (litegraph marks it `"widget": {...}`,
        # which is also what promoting one onto a subgraph does) keeps its slot in
        # widgets_values even though a wire now feeds it. Skipping that slot
        # shifts every later widget by one — an EmptySD3LatentImage with width and
        # height wired hands 1024 to batch_size, a KSampler with a promoted seed
        # gets the seed value as its sampler name. The graph still runs, which is
        # what makes it dangerous, so the cursor advances on wired widgets too.
        converted = {
            i.get("name") for i in (n.get("inputs") or [])
        ...
        for iname, ispec in ordered:
            ...
            wired = iname in conn
            holds_slot = _is_widget_type(itype) and (not wired or iname in converted)
            if wired:
                inputs[iname] = conn[iname]
            elif holds_slot and wi < len(widgets):
                inputs[iname] = _coerce(widgets[wi], itype)
            if holds_slot and wi < len(widgets):
                wi += 1
    ...
    # A widget promoted onto a subgraph instance belongs to an interior input that
    # positional mapping just filled with the definition's default. The value the
    # user actually typed wins.
    for nid, values in promoted.items():
        ...
        for name, value in values.items():
            ispec = fields.get(name)
            itype = ispec[0] if isinstance(ispec, list) and ispec else ispec
            node["inputs"][name] = _coerce(value, itype)
